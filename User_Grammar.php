...
<?php 														
    ...
    if (isset($_POST['click']) || isset($_GET['start'])) {
	    ...
	    $c = $_SESSION['clicks'];
			if(isset($_POST['userans'])) { $userselected = $_POST['userans'];
				...
				$fetchqry2 = "UPDATE `table_grammar` SET `userans`='$userselected' WHERE `id`=$c-1"; 
	...
	?>
    <br/><h1>Now it's time to check your knowledge</h1><br/>
...
<table><?php if(isset($c)) {   $fetchqry = "SELECT * FROM `table_grammar` where id='$c'"; 
...
<tr><td><h3><br><?php echo @$row['que'];?></h3></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 6){ ?>
  ...
  <tr><td><button class="btn btn-secondary" name="click" >Next</button></td></tr> <?php }  
																	?> 
  <form>
 <?php if($_SESSION['clicks']>5){ 
	$qry3 = "SELECT `ans`, `userans` FROM `table_grammar`;";
