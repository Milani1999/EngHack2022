    ...
    if (isset($_POST['click']) || isset($_GET['start'])) {
	    ...
	    $c = $_SESSION['clicks'];
			if(isset($_POST['userans'])) { $userselected = $_POST['userans'];
				...
				$fetchqry2 = "UPDATE `table_speaking` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<br/><h1>Challenge!! Now it's time for you to practise and perform</h1><br/>
...
<table><?php if(isset($c)) {   $fetchqry = "SELECT * FROM `table_speaking` where id='$c'"; 
	...
	<?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 6){ ?>
		<tr><td><audio controls><source src='uploaded_img/<?php echo $row['que']; ?>'></audio><br><br></td></tr>
			<tr><td><?php echo $row['que_read']; ?><br><br></td></tr>	
  <tr><td><input required type="radio" name="userans" value="<?php echo $row['option_1'];?>" class="form-check-input">&nbsp;<?php echo $row['option_1']; ?><br>
  <tr><td><input required type="radio" name="userans" value="<?php echo $row['option_2'];?>" class="form-check-input">&nbsp;<?php echo $row['option_2'];?></td></tr>

  <tr><td><br><button class="btn btn-secondary" name="click" >Next</button></td></tr> <?php }  
																	?>
                                                   
  <form>
 <?php if($_SESSION['clicks']>5){ 
	$qry3 = "SELECT `ans`, `userans` FROM `table_speaking`;";
...
<div class="p-3 mb-2 bg-danger text-white">
   <?php
      echo $x."  Better luck next time !! <br>";
      ?>
      </div>
 ...
 <h2>Result</h2>
 <span>Correct responses:&nbsp;<?php echo $no = @$_SESSION['score']; 
